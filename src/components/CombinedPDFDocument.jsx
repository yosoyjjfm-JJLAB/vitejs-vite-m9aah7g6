...
const CombinedPDFDocument = ({ tickets = [] }) => {
                ...
                <React.Fragment key={data.id || idx}>
                    ...
                    <Page size="A4" style={styles.page}>
                        <View style={styles.header}>
                            ...
                            <View style={styles.invoiceInfo}>
                                <Text style={[styles.label, { color: '#3b82f6', fontWeight: 'bold' }]}>
                                    {data.serviceType?.toUpperCase() || 'SERVICIO TÉCNICO'}
                                </Text>
                        ...
                        <View style={{ flexDirection: 'row', marginTop: 20 }}>
                            <View style={{ width: '50%', paddingRight: 10 }}>
                                ...
                                <Text style={styles.text}>{data.customerName || data.customer || 'Sin Nombre'}</Text>
                                {(data.customerCompany || data.company) && (
                                    <Text style={[styles.text, { fontWeight: 'bold' }]}>{data.customerCompany || data.company}</Text>
                                )}
                                {data.customerPuesto && (
                            ...
                            <View style={{ width: '50%', paddingLeft: 10 }}>
                                ...
                                <Text style={styles.text}>
                                    <Text style={{ fontWeight: 'bold' }}>Tipo:</Text> {data.deviceType || 'Equipo'}
                                </Text>
                                <Text style={styles.text}>
                                    <Text style={{ fontWeight: 'bold' }}>Modelo:</Text> {data.deviceModel || data.device || 'N/A'}
                                </Text>
                                <Text style={styles.text}>
                                    <Text style={{ fontWeight: 'bold' }}>Serie:</Text> {data.deviceSerial || 'N/A'}
                                </Text>
                            </View>
                        </View>
                        ...
                        <View style={{ marginBottom: 15 }}>
                            ...
                            <Text style={styles.text}>{data.problemDescription || 'Sin descripción de falla.'}</Text>
                        </View>
                        ...
                        <View style={styles.footer}>
                            <Text style={styles.footerText}>
                                JJLAB Tecnología Creativa — Equipo #{data.id ? data.id.slice(-6).toUpperCase() : idx + 1}
                            </Text>
                        </View>
                    </Page>
                    ...
                    <Page size="A4" style={styles.page}>
                        ...
                        <View style={styles.statusBadge}>
                            <Text style={styles.statusText}>ESTADO: {(data.status || 'PENDIENTE').toUpperCase()}</Text>
                        </View>

                        {data.showSignature !== false && (
                            <View style={styles.signatureContainer} wrap={false}>
                                <Image
                                    src={origin + '/firma.png'}
                    ...
                    {/* PÁGINA 3: Evidencia Fotográfica (si existe) */}
                            ...
                            <View style={styles.header}>
                                <View style={styles.logoSection}>
                                    ...
                                    <Text style={styles.subLogoText}>
                                        Evidencia del Servicio #{data.id ? data.id.slice(-6).toUpperCase() : ''} - {data.deviceModel || data.device || ''}
                                    </Text>
                                </View>
                            </View>

                            <View style={styles.galleryGrid}>
                                {data.photos.map((photoUrl, pIdx) => (
                                    <View key={pIdx} style={styles.galleryItem}>
